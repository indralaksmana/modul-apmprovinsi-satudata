            ...
            <table ref="table" class="table">
                <thead>
                    <tr>
                        <th v-for="(column, index) in columns" @click="sort(index)" :class="(sortable ? 'sortable' : '')
                            + (sortColumn === index ?
                                (sortType === 'desc' ? ' sorting-desc' : ' sorting-asc') : '')" :style="{width: column.width ? column.width : 'auto', 'vertical-align' : 'middle'}" :rowspan="column.rowspan" :colspan="column.colspan">
                            {{column.label}} <i class="fa float-right" :class="(sortColumn === index ?
                                (sortType === 'desc' ? ' ti-angle-down' : ' ti-angle-up') : '')"></i>
                        </th>
                        <th v-for="(column2, index) in columns2" @click="sort(index)" :class="(sortable ? 'sortable' : '')
                            + (sortColumn === index ?
                                (sortType === 'desc' ? ' sorting-desc' : ' sorting-asc') : '')" :style="{width: column2.width ? column2.width : 'auto', 'text-align' : 'center'}" :rowspan="column2.rowspan" :colspan="column2.colspan">
                            {{column2.label}} <i class="fa float-right" :class="(sortColumn === index ?
                                (sortType === 'desc' ? ' ti-angle-down' : ' ti-angle-up') : '')"></i>
                        </th>
                        <slot name="thead-tr"></slot>
                    </tr>
                    <tr>
                         <th v-for="(column3, index) in columns3">
                            {{column3.label}}
                        </th>
                        <slot name="thead-tr"></slot>
                    </tr>
                ...
                <tbody>
                    <tr v-for="(row, index) in paginated" @click="click(row, index)">
                        ...
                        <template v-for="column3 in columns3">
                            <td :class="column3.numeric ? 'numeric' : ''" v-if="!column3.html">
                                {{ collect(row,column3.field) }}
                            </td>
                            <td :class="column3.numeric ? 'numeric' : ''" v-html="collect(row, column3.field)" v-if="column3.html">
                            </td>
                        </template>
